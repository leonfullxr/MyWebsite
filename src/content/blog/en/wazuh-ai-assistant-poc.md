...
3. **Datastore-computed counts**: every "how many" answer comes from `total_matching` or aggregation buckets that OpenSearch computed. The sampled alert list handed to the model is explicitly truncated. Counting it is impossible by construction.
4. **Zero-hit differential diagnosis**: when a query returns nothing, the pipeline probes the time window and each filter individually. The answer can distinguish "verified: 214 documents in the window, none from agent db-99" from "the query was wrong".

Finally, every claim in a synthesized answer must cite an `[alert:id]`, `[agg:name]` or `[kb:id]` identifier. The service verifies each citation against what was actually retrieved. An invented citation surfaces as a correction event instead of a confident lie. Every answer carries a verifiability label stating which lane produced it and which checks ran.
...
The division of labor matters. The dashboard and the other edges are front doors and nothing more. The brain is a headless **tool service** (the gateway) that owns the entire agent loop. It resolves which environment a request belongs to from a per-environment key. It exposes the read lanes, the veracity pipeline, the action layer and the audit trail. A per-tool HTTP surface executes exactly one validated tool with no model involved. That is what the deterministic evals drive.

## The identity chain: the AI queries as you

The property I wanted to show is that the reasoning core can never forge an identity and holds no standing credential that reads telemetry. The chain has four hops and each hop verifies the previous one. Importantly, it needs no external identity provider.

The analyst presents their existing Wazuh credentials. A dedicated **auth-shim** sidecar verifies those credentials against the environment's *own* indexer, by calling its `authinfo` endpoint. Identity is whatever the Wazuh security plugin already trusts — internal users, LDAP, or SSO — with nothing new to stand up. The shim confirms the user carries the analyst role and mints a turn credential: an RS256 JWT with a dual audience, a lifetime of at most ten minutes, and a tenant claim that comes from *which* environment's `authinfo` accepted the login rather than from anything in the request. The shim is the only container holding the signing key. The tool service verifies with the public key only. A compromised reasoning core still cannot mint identities.

The fourth hop is my favorite part. The Wazuh indexer's security configuration gains a JWT auth domain trusting the same public key. The tool service forwards the analyst's own token and the indexer resolves it to a read-only role scoped to alert indices. Telemetry queries execute **as the logged-in analyst**. You can prove the ceiling directly, with no AI in the loop:
...
The two model tiers (a small router model for cheap decisions, a larger analysis model for the investigation loop) can each bind to a different provider. A local router with Bedrock analysis is a two-line configuration. For local serving, sparse mixture-of-experts models turned out to be the practical unlock. `gpt-oss:20b` gives big-model quality with about 3.6B active parameters and runs on a 16 GB machine. `qwen3:30b-a3b` fits entirely in 24 GB of VRAM. At the extreme end I wired an experimental depth lane around AirLLM-style layer streaming. It really does run 70B-class models on a 4 GB GPU, at 0.07 to 0.7 tokens per second. That is not an interactive assistant and no configuration makes it one. It is honestly positioned as a batch lane for one hard question overnight, never the chat path.

## When the logs attack back

A SIEM assistant has an unusual threat model. Its most dangerous input is not the user's question. It is the evidence. Alert bodies contain whatever an attacker managed to write into a log line. Every piece of evidence the model reads is potentially an adversarial instruction. The pipeline treats it that way. The analyst's question passes a prompt-attack filter. Retrieved evidence passes its own guardrail before the model sees it. The model's output passes a third check for secrets, PII and grounding. Citation verification then strips and flags any claim that references evidence that was never retrieved. What reaches the browser is sanitized Markdown: no HTML, no external links, no auto-loading images.

The honest defense, though, is structural. Every tool the model can call is read-only and tenant-scoped. Even a perfectly successful injection has nothing dangerous to hijack. Guardrail interventions and citation failures are audit events in the tenant's own indexer. An injection *attempt* becomes a SOC detection about the attacker. The assistant turns the attack into telemetry.

## Recognition before reasoning

The optimization I like most needed no GPU at all. Analysts ask the same operational questions constantly. Those questions do not need a reasoning model. Lane 0 embeds each incoming question with a small local embedding model (`bge-m3`, which handles English and Spanish in one space). It matches against curated exemplars by cosine similarity. It extracts slots like time windows and agent names with deterministic bilingual rules. It executes the matched template through the exact same veracity pipeline as every other lane. A hit answers in tens of milliseconds with zero model tokens. On Bedrock that literally means the most frequent questions cost nothing. A miss escalates silently. Lane 0 can never break the assistant. It can only relieve it.

A single cosine threshold, though, will eventually fire the wrong template on a paraphrase with a negation in it. A hit needs more than clearing 0.80. The near-miss band between 0.65 and 0.80 does not execute a template. Instead the closest exemplar is injected as a transient few-shot hint for the model, after the cache-stable prompt prefix so it never poisons prompt caching. Recognition that is *almost* sure helps the model without pretending to be certain.

Next to it sits an evidence cache keyed on a hash of the canonical query plan. Time bounds are floored to a TTL grid so that "last 24 hours" asked twice in the same minute is one query, not two. Cached answers disclose `served_from_cache`. An assistant built on verifiability does not get to hide its shortcuts.

## The knowledge lane, now shipped

Analysts do not only ask about telemetry. They also ask "what does rule 5710 mean" and "how do I enroll an agent". Those questions deserve the same veracity treatment as questions about alerts. The assistant now has a knowledge lane over the official Wazuh documentation: a version-pinned corpus, ingested from the docs, chunked and embedded. `knowledge_search` retrieves against it. Every passage it returns is cited as `[kb:id]` and verified against what was actually retrieved, exactly like `[alert:id]`. This is the one place the system embeds anything, and it is deliberate. The only vector store in the whole design is over *public reference content*, never over a tenant's telemetry. Tenant data is queried, never embedded.
...
The other milestone that has shipped is write actions. Sooner or later every assistant gets asked to *do* something — restart that agent, suppress that noisy rule — and the rule that keeps it safe is structural, like everything else: the reasoning core holds zero credentials that can perform actions. When the model emits a `propose_action`, the gateway validates it against an allowlist. It verifies the target independently by querying the indexer as the analyst. It renders a card of facts the model never wrote. Nothing executes until a human confirms. Execution runs under a per-tier least-privilege credential the model never holds — dashboard, manager, or active-response, each its own scoped identity. High-risk tiers demand the approver re-type the target. The operations half of this — capacity, the full action flow, audit and incident response — is its own post: [Operating an AI Assistant in a SOC](/en/blog/operating-wazuh-ai-soc/).

## Proving it works instead of demoing it

Demos convince nobody. The harness turns its claims into assertions. A seeder writes about two thousand synthetic alerts with a deterministic seed and records the exact ground truths into a file. A bilingual golden set of 32 evaluation cases then runs against the live stack through the full chain, from login to the final answer. It asserts tool selection, ground-truth counts, zero-hit honesty, prompt-injection resistance, and the absence of unverified citations. It exits nonzero on any failure. That makes it a CI gate: prompt changes become physically unable to merge unevaluated. Separate suites drive the write-action paths (`make evals-actions`) and the dashboard connector edge (`make evals-connector`). A unit suite that has grown from 26 to more than 150 tests covers the deterministic core — IR validation, DSL compilation, lane 0 slot extraction, cache keying and the citation checks.

The operational edges got the same treatment. Answers stream token by token. Capacity is a bounded queue that rejects honestly instead of degrading silently. A kill switch turns every surface into a 503 when needed. A Prometheus endpoint exposes turns by lane, tool outcomes and latency histograms.
...
In production the harness unfolds into EKS with one namespace per environment behind a default-deny NetworkPolicy. Every environment gets its own IAM role (IRSA), its own Bedrock inference profile and guardrail, and its own KMS key. The AI path never touches the internet. Bedrock, STS, Secrets Manager and logging are all reached through VPC interface endpoints. Audit lands both in the environment's own indexer and in an Object Lock S3 bucket. The only shared hop on the data path is Bedrock's stateless model fleet. That is exactly the hop with no retention.

## What I would harden next

A PoC earns the right to be taken seriously by knowing its own gaps. Here is my list, in order:

- **The evidence cache must key on identity, not just on the query plan.** Two analysts with different index permissions must never share a cache entry. The effective role set belongs in the cache key. It is the kind of bug that only exists because the cache works.
...
- **Local models deserve constrained decoding.** Grammar-constrained output for the Query IR turns parse-and-retry loops into first-try validity. On a 20B local model that is real latency back.
...
The two additions I originally flagged as milestones — a knowledge lane over the Wazuh docs, and human-approved actions — are the two sections above, now shipped and live-validated. Multi-turn conversations survive across a turn now too. Follow-ups that reference earlier evidence stay verifiable. What remains genuinely open, rather than merely unbuilt, is the multi-environment cross-tenant isolation suite. The shape is there and the registry is real. Asserting isolation across live tenants is the work I set aside to keep the self-hosted story sharp.

## What I took away

Three lessons survived contact with the implementation. First, veracity is structural or it is nothing. Allowlists, server-side compilation, datastore-computed counts and verified citations do more for trust than any amount of prompt engineering. They hold even when the model is wrong. Second, identity is the feature nobody demos and everybody needs. Queries-as-user through a JWT auth domain means the AI inherits exactly the permissions of the person asking. That single property answers most of the hard multi-tenancy questions before they are asked. Third, honest engineering beats impressive engineering. The layer-streaming depth lane is technically the flashiest part of the stack. The most valuable thing I did with it was measure it, state that 0.2 tokens per second is not a chat experience, and confine it to the lane where it genuinely helps.
