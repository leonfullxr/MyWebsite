...
date: "2024-07-24"
description: "My first real self-hosted service: Nextcloud on a Raspberry Pi with Docker Compose, PostgreSQL, Redis, Traefik with Let's Encrypt, Imaginary previews, and client push via notify_push."
...
The healthchecks are not decoration. The `app` container declares `depends_on` with `condition: service_healthy`. Nextcloud never starts against a database that is not actually accepting connections yet. That single line eliminates the classic "Internal Server Error on first boot" that fills the forum threads.

The `app` and `cron` containers share the same bind mounts (`./nextcloud`, `./config`, `./apps`, `./data`). A single image upgrade moves both in lockstep.

## notify_push, the part worth stealing

Without Client Push, every Nextcloud client polls the server on a timer. That is slow to react and wasteful on a Pi. The `notify_push` app ships a small Rust daemon. It holds a websocket open to every client and tells them the instant something changes. Getting it running in Compose is the part most guides skip. It is exactly what wwe's thread solves:
...
- **The binary path is architecture-specific.** The app ships prebuilt binaries. On a Pi it is `bin/aarch64/notify_push`. The `x86_64` path from most examples fails with a cryptic "no such file" inside a container that plainly has files.
...
- **It will crash-loop on first boot, and that is fine.** The binary does not exist until the app is installed inside Nextcloud. The sequence is: bring the stack up, install the app, restart the service, then wire it up:
...
- **PHP sessions belong in Redis.** The official image stores sessions on disk, which fights with multiple containers sharing the volume. A one-file fix mounted into the PHP config directory ([context](https://github.com/nextcloud/docker/issues/182)) points session storage at the `redis` service. Logins stop mysteriously expiring.
...
- **HSTS and the https redirect live in Traefik**, not Nextcloud. The `secHeaders` and `redirectScheme` middlewares above handle them. Nextcloud's own security scan goes green once they are in place.
- **First boot takes minutes.** The container reports "started" long before initialization finishes. Patience beats re-creating the stack because the login page is not up after thirty seconds.

## The dynamic IP problem (a preview)

The one wart on this setup: my ISP rotates my public IP. The Cloudflare `A` record has to follow it. For now a small script on a systemd timer checks the public IP daily. It updates the DNS record through the Cloudflare API when it drifts, and resets Traefik's ACME storage so a fresh certificate gets issued. It works, but it is the part of the architecture I trust the least. The original guide teases a Cloudflare Zero Trust integration. I suspect that is the direction this server eventually heads.

For now though: files sync, calendars share, photos upload from the phone, previews render fast thanks to imaginary, and the desktop client updates the instant a file changes anywhere. For a credit-card-sized computer in a drawer, that is a very complete personal cloud.
