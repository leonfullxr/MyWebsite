...
date: "2026-07-10"
description: "Moving a photo library across CPU architectures without converting the database, why version pinning saved the migration, and the VirtualBox incident that showed where bulk data does and does not belong."
...
Nextcloud, doing exactly what it promises, treated them like documents. Every tweak to a VM created a new *version* of a multi-GB file. Every delete moved one to the *trashbin*. Result: **hundreds of gigabytes of invisible bloat**. None of it was visible in the files UI. All of it was faithfully preserved.
...
Two systems, one lesson. Nextcloud went wrong when I put bulk binaries into managed, versioned, synced storage. Immich went right because the bulk bytes lived in a plain folder that could be *carried across a room*. The small hot state (the database) was portable by discipline: pinned versions, compatible formats, a rollback copy.
