...
date: "2026-07-10"
description: "Mirroring a 2,900-track Spotify library onto a Raspberry Pi with Navidrome and spotdl: metadata banking, resumable backfills, playlist reconstruction from ID3 tags, and what Spotify's rate limiter taught me."
...
[spotdl](https://github.com/spotDL/spotify-downloader) never touches Spotify's audio. That is DRM-protected. Spotify is used purely as a *metadata oracle*: which tracks, which albums, what artwork, what track numbers. The audio itself comes from YouTube Music. It streams without DRM, matched per track by searching "artist title album". The ceiling is YouTube Music's ~128 kbps opus (converted to mp3). That is fine on phone speakers and commutes. It is not an archival format.

Navidrome's side of the bargain is beautifully boring. It watches a folder. Files appear as `{Artist}/{Album}/{NN} - {Title}.mp3`. An hourly scan picks them up. Dozens of mobile apps that speak the Subsonic API (Symfonium, Tempo, Amperfy...) stream them through the [Cloudflare Tunnel](/en/blog/from-port-forwarding-to-cloudflare-tunnels/):
...
The initial bulk download deserved paranoia. The failing request was not per-track. It was the session fetch that happens *once per spotdl invocation*. The loop was built around that:
...
- The Web API **cannot see playlist folders**. The internal endpoint that has them answers `403` to normal OAuth tokens. A first-party web-player token (borrowed from the browser's DevTools) can fetch it. Folder names become display prefixes like `T0P / rap`.
- Navidrome reads a playlist's `#PLAYLIST:` name **only on first import**. It does **not** auto-prune a playlist when its `.m3u` file disappears. Renaming or removing one properly means a manual database prune.
...
Entirely. The nightly timer keeps the mirror fresh. Playlists refill themselves as stragglers get retried. My phone streams my own library from my own hardware through my own tunnel. Spotify is reduced to the role it is genuinely best at: knowing what music I like.
